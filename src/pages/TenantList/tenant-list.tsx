...
import { Calendar, Database, ExternalLink, Globe, Server, Shield, User } from "lucide-react"
...
export default function TenantList() {
  ...
  useEffect(() => {
    notify.success("hello welcome to OpsBeat");
    }, []);
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">View and manage all tenant configurations in the system</p>
      </div>
<div className="h-[calc(100vh-200px)] overflow-auto">
      <div className="grid gap-6">
        {tenants.map((tenant) => (
          <Card key={tenant.id} className="w-full overflow-hidden transition-all hover:shadow-md">
            <CardHeader className="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <CardDescription className="mt-1">Tenant ID: {tenant.tenantId}</CardDescription>
                </div>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">{tenant.dbName}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">{tenant.bucketName}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1 truncate max-w-[200px]">{tenant.cloudfrontUrl}</p>
                  </div>
                </div>
              </div>
            </CardContent>
            <CardFooter className="bg-slate-50 dark:bg-slate-900/50 px-6 py-4 flex justify-between">
              <div className="text-sm text-muted-foreground">Created: {formatDate(tenant.createdOn)}</div>
              <Button variant="outline" onClick={() => handleShowMore(tenant)} className="gap-2">
                Show More Details
        ...
        {selectedTenant && (
          <DialogContent className="max-w-5xl w-5xl h-[calc(100vh-50px)] overflow-auto sm:max-w-5xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Complete configuration details for tenant ID: {selectedTenant.tenantId}
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="grid md:grid-cols-2 gap-x-12 gap-y-6">
                ...
                <div>
                  ...
                  <dl className="grid grid-cols-[120px_1fr] gap-2 text-sm">
                    ...
                    <dd>{selectedTenant.updatedOn ? formatDate(selectedTenant.updatedOn) : "Never"}</dd>
                  </dl>
                </div>
              </div>
